def main():
    ...
    formula_mass = float(input("Enter the mass in grams of the sample: "))

    # Call the make_periodic_table function and
    # store the periodic table in a variable.
    per_table = make_periodic_table()

    # Call the parse_formula function to convert the
    # chemical formula given by the user to a compound
    # list that stores element symbols and the quantity
    # of atoms of each element in the molecule.
    symbol_quantity_list = parse_formula(chemical_formula, per_table)

    # Call the compute_molar_mass function to compute the
    # molar mass of the molecule from the compound list.
    individual_molar_masses = {}
    molar_mass = compute_molar_mass(symbol_quantity_list, per_table)

    # Compute the number of moles of the sample.
    formula_moles = formula_mass / molar_mass

    # Print the molar mass.
    print(f"{molar_mass:.5f} grams/mole {chemical_formula}")

    # Print the number of moles.
    print(f"{formula_moles:.5f} moles {chemical_formula}")

    # EXTRA: Print the number of moles and grams of each element in the sample.
    for element_symbol, element_quantity in symbol_quantity_list:
        element_moles = element_quantity * formula_moles
        print(f"{per_table[element_symbol][NAME_INDEX]}: {element_moles:.5f} moles, "
        f"{element_moles * per_table[element_symbol][ATOMIC_MASS_INDEX]:.5f} grams.")
...
def compute_molar_mass(symbol_quantity_list, periodic_table):
    ...
    for symbol, quantity in symbol_quantity_list:
        total_mass += periodic_table[symbol][ATOMIC_MASS_INDEX] * quantity
    return total_mass
